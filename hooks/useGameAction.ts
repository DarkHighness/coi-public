...
export const useGameAction = ({
  ...
  const handleAction = useCallback(async (
    ...
    console.log("[useGameAction] handleAction called", { action, isInit, forceTheme, fromNodeId, preventFork });
    ...
    if (!isInit && parentId) {
      ...
      if (!reuseExistingNode && preventFork) {
        const siblingNode = Object.values(gameStateRef.current.nodes).find(
          (n) => n.parentId === parentId && n.role === "user" && n.text === action
        );
        ...
        if (siblingNode) {
          ...
          reuseExistingNode = true;
          console.log(`[HandleAction] Reusing existing sibling node ${siblingNode.id} for retry`);
        }
      }
    }
