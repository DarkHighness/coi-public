...
export const inventoryItemHiddenSchema = z.object({
  ...
  secrets: z.array(z.string()).optional().describe("Hidden secrets about the item."),
});
...
export const inventoryItemSchema = z.object({
  ...
  highlight: z.boolean().optional().describe("True when updated in current turn (for UI)."),
...
export const relationshipVisibleSchema = z.object({
  ...
  description: z.string().describe("Public perception - how others view this NPC."),
  ...
  currentImpression: z.string().optional().describe(
    "The NPC's current state from the protagonist's perspective."
  ),
  personality: z.string().optional().describe(
    "Public perception of personality - what people SAY about them."
  ),
  affinity: z.number().int().min(0).max(100).describe(
    "Affinity score 0-100. <30=hostile, 30-70=neutral, >70=friendly."
  ),
  affinityKnown: z.boolean().optional().describe(
    "Whether the player knows the affinity level."
  ),
});

/** 关系隐藏层 */
export const relationshipHiddenSchema = z.object({
  trueName: z.string().optional().describe("The character's real name (if different)."),
  realPersonality: z.string().describe("True personality - what they REALLY are like."),
...
export const relationshipSchema = z.object({
  ...
  currentLocation: z.string().optional().describe(
    "The NPC's current location ID (e.g., 'loc:1')."
  ),
  visible: relationshipVisibleSchema,
  hidden: relationshipHiddenSchema,
  notes: z.string().optional().describe(
    "NPC's observations of player's displayed knowledge/behavior."
  ),
  unlocked: z.boolean().optional().describe(
...
export const locationVisibleSchema = z.object({
  ...
  knownFeatures: z.array(z.string()).describe("Known features of the location."),
});

/** 地点隐藏层 */
export const locationHiddenSchema = z.object({
  fullDescription: z.string().describe("True nature of the location."),
  hiddenFeatures: z.array(z.string()).describe("Hidden features not yet discovered."),
...
export const questHiddenSchema = z.object({
  trueDescription: z.string().optional().describe("The hidden truth or real purpose."),
  trueObjectives: z.array(z.string()).optional().describe("True hidden objectives."),
  secretOutcome: z.string().optional().describe("Secret outcome if quest is completed."),
});
...
export const questSchema = z.object({
  ...
  unlocked: z.boolean().optional().describe(
    "AI DECISION: Set true when quest's hidden purpose is revealed."
  ),
  highlight: z.boolean().optional(),
  createdAt: z.number().optional(),
  lastModified: z.number().optional(),
...
export const skillHiddenSchema = z.object({
  ...
  hiddenEffects: z.array(z.string()).describe("Hidden effects not yet discovered."),
  drawbacks: z.array(z.string()).optional().describe("Hidden drawbacks or costs."),
});
...
export const conditionVisibleSchema = z.object({
  ...
  perceivedSeverity: z.string().optional().describe("How severe it appears to be."),
});

/** 条件隐藏层 */
export const conditionHiddenSchema = z.object({
  trueCause: z.string().describe("The true cause of this condition."),
  actualSeverity: z.number().int().optional().describe("Actual severity level."),
  progression: z.string().optional().describe("How the condition will progress."),
...
export const conditionSchema = z.object({
  ...
  type: conditionTypeSchema.describe("Condition type: buff, debuff, or neutral."),
  ...
  unlocked: z.boolean().optional().describe(
    "AI DECISION: Set true when true cause/cure revealed."
  ),
  highlight: z.boolean().optional(),
});
...
export const knowledgeHiddenSchema = z.object({
  ...
  misconceptions: z.array(z.string()).optional().describe("Common misconceptions."),
  toBeRevealed: z.array(z.string()).optional().describe("Info to be revealed later."),
});
...
export const knowledgeEntrySchema = z.object({
  ...
  unlocked: z.boolean().optional().describe(
    "AI DECISION: Set true when full truth discovered."
  ),
  highlight: z.boolean().optional(),
  createdAt: z.number().optional(),
  lastModified: z.number().optional(),
...
export const timelineEventVisibleSchema = z.object({
  ...
  causedBy: z.string().optional().describe("Publicly known cause or instigator."),
});

/** 时间线事件隐藏层 */
export const timelineEventHiddenSchema = z.object({
  trueDescription: z.string().describe("The true nature of the event (GM knowledge)."),
  trueCausedBy: z.string().optional().describe("The real instigator or cause."),
  consequences: z.array(z.string()).optional().describe("Hidden consequences or future implications."),
});
...
export const pendingConsequenceSchema = z.object({
  ...
  readyAfterTurn: z.number().int().describe(
    "The consequence CAN'T trigger UNTIL after this turn number."
  ),
  createdAtTurn: z.number().int().optional().describe("Turn when this consequence was created."),
  conditions: z.array(z.string()).optional().describe(
    "Narrative conditions you'll check when deciding to trigger."
  ),
  triggered: z.boolean().optional().describe("True once consequence has been triggered."),
...
/** 因果链状态 */
export const causalChainStatusSchema = z.enum(["active", "resolved", "interrupted"]);

/** 完整因果链 Schema */
export const causalChainSchema = z.object({
  chainId: z.string().describe("Format: chain:N"),
  rootCause: rootCauseSchema,
  events: z.array(timelineEventSchema).optional().describe("Events in this chain."),
  status: causalChainStatusSchema.describe("Current status of the chain."),
  pendingConsequences: z.array(pendingConsequenceSchema).optional().describe(
...
export const factionSchema = z.object({
  ...
  unlocked: z.boolean().optional().describe("True when secret agenda is revealed."),
  highlight: z.boolean().optional(),
});
...
/** 角色属性颜色 */
export const attributeColorSchema = z.enum(["red", "blue", "green", "yellow", "purple", "gray"]);

/** 角色属性 */
export const characterAttributeSchema = z.object({
  label: z.string().describe("Name of attribute (e.g. Health, Sanity, Credits)."),
...
export const hiddenTraitSchema = z.object({
  ...
  triggerConditions: z.array(z.string()).optional().describe("Conditions to trigger the trait."),
  unlocked: z.boolean().describe(
...
export const characterStatusSchema = z.object({
  ...
  attributes: z.array(characterAttributeSchema).describe("Character attributes."),
  skills: z.array(skillSchema).describe("Character skills."),
  conditions: z.array(conditionSchema).describe("Active conditions."),
  hiddenTraits: z.array(hiddenTraitSchema).optional().describe("Hidden personality traits."),
  appearance: z.string().describe("Detailed physical appearance."),
  profession: z.string().optional().describe("Character's occupation or class."),
  background: z.string().optional().describe("Brief life story and background."),
...
export const mainGoalSchema = z.object({
  ...
  hidden: z.string().optional().describe("The hidden event logic or true nature."),
});
...
export const atmosphereSchema = z.object({
  envTheme: envThemeSchema.describe("The visual theme/color palette for UI rendering."),
...
export const storyOutlineSchema = z.object({
  ...
  quests: z.array(questSchema).describe("Initial quests (at least one main quest is required)."),
  worldSetting: worldSettingSchema.describe("Dual-layer world setting."),
  factions: z.array(factionSchema.omit({ id: true })).describe("Major power groups or factions."),
  locations: z.array(locationSchema.omit({ id: true, isVisited: true, createdAt: true })).describe(
    "Initial locations with full details."
  ),
  knowledge: z.array(knowledgeEntrySchema.omit({ id: true })).optional().describe(
    "Initial knowledge entries about the world."
  ),
  timeline: z.array(timelineEventSchema).optional().describe(
    "Initial timeline events representing the backstory."
  ),
  character: characterStatusSchema.describe("The initialized character profile."),
  inventory: z.array(inventoryItemSchema.omit({ id: true })).optional().describe(
    "Initial items in the inventory (1-3 items)."
  ),
  relationships: z.array(relationshipSchema.omit({ id: true })).optional().describe(
    "Initial relationships (1-2 NPCs)."
  ),
...
export const summaryVisibleSchema = z.object({
  ...
  majorEvents: z.array(z.string()).describe("List of major events player witnessed."),
  characterDevelopment: z.string().describe("Character development from player's view."),
  worldState: z.string().describe("World state as player understands it."),
});

/** 摘要隐藏层 */
export const summaryHiddenSchema = z.object({
  truthNarrative: z.string().describe("Objective truth narrative of what really happened."),
  hiddenPlots: z.array(z.string()).describe("Hidden plots developing in the background."),
  npcActions: z.array(z.string()).describe("NPC actions player didn't witness."),
...
export const gameResponseSchema = z.object({
  ...
  choices: z.array(z.string()).min(2).max(4).describe(
    "2-4 options for the player's next action."
  ),
  ...
  inventoryActions: z.array(z.object({
  ...
  })).optional().describe("Updates to inventory."),
  relationshipActions: z.array(z.object({
    ...
    notes: z.string().optional(),
    unlocked: z.boolean().optional(),
  })).optional().describe("Updates to relationships."),
  locationActions: z.array(z.object({
    ...
    notes: z.string().optional(),
    unlocked: z.boolean().optional(),
  })).optional().describe("Updates to locations."),
  questActions: z.array(z.object({
    ...
    unlocked: z.boolean().optional(),
  })).optional().describe("Updates to quests."),
  knowledgeActions: z.array(z.object({
    ...
    unlocked: z.boolean().optional(),
  })).optional().describe("Updates to knowledge."),
  factionActions: z.array(z.object({
    action: z.enum(["update"]),
    id: z.string(),
    name: z.string(),
    visible: z.string().optional(),
    hidden: z.string().optional(),
  })).optional().describe("Background actions taken by factions this turn."),
  characterUpdates: z.object({
  ...
  }).optional().describe("Updates to character stats, skills, and profile."),
  timelineEvents: z.array(z.object({
    ...
    projectedConsequences: z.array(z.object({
      description: z.string(),
...
export { zodToGemini, zodToOpenAIResponseFormat, zodToOpenAISchema } from "./zodCompiler";
