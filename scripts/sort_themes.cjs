...
async function sortThemes() {
  try {
    ...
    const startMarker = 'export const THEMES: Record<string, StoryThemeConfig> = {';
    ...
    const themesBody = themesTsContent.substring(startIndex + startMarker.length, endIndex);
    ...
    for (const line of lines) {
      ...
      if (inBlock) {
        ...
        if (braceCount === 0) {
          ...
          const categoriesMatch = currentBlock.match(/categories:\s*\[([\s\S]*?)\]/);
          let categories = [];
          if (categoriesMatch) {
            categories = categoriesMatch[1].split(',').map(c => c.trim().replace(/['"]/g, ''));
            ...
            currentBlock = currentBlock.replace(/categories:\s*\[[\s\S]*?\]/, newCategoriesStr);
          }
          ...
          if (isIp && !hasRestricted) {
             ...
             newContent = newContent.replace(/^\s*restricted:\s*true,?\s*$\n?/gm, '');
          }
    ...
    const newContent = themesTsContent.substring(0, startIndex + startMarker.length) +
                       newThemesBody +
