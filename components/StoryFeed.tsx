...
export const StoryFeed: React.FC<StoryFeedProps> = ({
  ...
  useEffect(() => {
    if (layout === 'scroll' && scrollContainerRef.current && contentRef.current) {
  ...
  const safeActiveIndex = Math.min(activeIndex, Math.max(0, currentHistory.length - 1));
        ...
        <div ref={contentRef} className="flex flex-col min-h-full">
          ...
          {gameState.outline && (
             <div className="mb-8 p-6 bg-theme-surface-highlight/20 border border-theme-primary/30 rounded-lg mx-auto max-w-3xl text-center animate-fade-in">
                 <h3 className="text-theme-primary font-fantasy text-xl mb-2">{gameState.outline.title}</h3>
                 <p className="text-theme-muted text-sm italic">{gameState.outline.premise}</p>
             </div>
          )}
            ...
            // Stack Layout
             <div className="flex-1 flex flex-col justify-center items-center relative min-h-[400px] w-full">
               {currentHistory.length > 0 && activeSegment && (
                  <div className="w-full max-w-3xl relative pb-24"> {/* Padding bottom for fixed controls */}
