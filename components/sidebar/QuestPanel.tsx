        ...
        <div className="space-y-4">
          ...
          {mainQuests.map((q) => (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <svg
                  className={`w-4 h-4 text-theme-primary transition-transform duration-200 ${expandedQuests.has(q.id) ? "rotate-180" : ""}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
          ...
          {sideQuests.length > 0 && (
            <div className="space-y-2">
              <h5 className="text-[10px] uppercase text-theme-muted tracking-widest border-b border-theme-border pb-1">
                {t("questPanel.side")}
              </h5>
              {sideQuests.map((q) => (
                <div
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <svg
                      className={`w-3 h-3 transition-transform duration-200 ${expandedQuests.has(q.id) ? "rotate-180" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  {expandedQuests.has(q.id) && (
                    <p className="mt-2 animate-[fade-in_0.3s_ease-in]">
                      {q.description}
                    </p>
                  )}
                </div>
          ...
          {activeQuests.length === 0 && (
            <p className="text-theme-muted text-sm italic p-2 text-center opacity-50">
              {t("questPanel.empty")}
            </p>
          )}
        </div>
