            ...
            recentEvents.map((event) => (
              <div
                ...
                className={`relative pl-3 border-l border-theme-border/50 pb-2 transition-all duration-300 ${
                  event.highlight ? "bg-yellow-900/10 -ml-1 pl-4 rounded pr-1" : ""
                }`}
              >
                <div className={`absolute left-[-1.5px] top-1.5 w-0.5 h-0.5 rounded-full ${event.highlight ? "bg-yellow-400 animate-pulse scale-150" : "bg-theme-primary"}`}></div>
                ...
                {event.unlocked && (
                  <div className="mt-2 text-xs border-l-2 border-yellow-500/50 pl-2 bg-yellow-900/10 py-1.5 rounded-r">
                    ...
                    {event.hidden?.trueDescription && (
                      <p className="leading-relaxed text-yellow-200/90 text-[10px] mb-1">{event.hidden.trueDescription}</p>
                    )}
                    {event.hidden?.trueCausedBy && (
                      <p className="text-[10px] text-yellow-200/80 italic mb-1">
                    ...
                    {event.hidden?.consequences && event.hidden.consequences.length > 0 && (
                      <div className="mt-1">
                        <span className="text-[9px] uppercase tracking-wider text-yellow-400/70 block mb-0.5">{t("hidden.consequences")}:</span>
