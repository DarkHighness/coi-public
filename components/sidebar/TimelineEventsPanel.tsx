            ...
            recentEvents.map((event) => (
              <div
                ...
                className={`relative pl-3 border-l border-theme-border/50 pb-2 transition-all duration-300 ${
                  event.highlight
                    ? "bg-theme-primary/10 -ml-1 pl-4 rounded pr-1"
                    : ""
                }`}
              >
                <div
                  className={`absolute left-[-1.5px] top-1.5 w-0.5 h-0.5 rounded-full ${event.highlight ? "bg-theme-primary animate-pulse scale-150" : "bg-theme-primary"}`}
                ></div>
                <div className="text-[10px] text-theme-muted font-mono mb-0.5 flex justify-between items-center">
                  <span>{event.gameTime}</span>
                  {event.unlocked && (
                    <svg
                      className="w-2.5 h-2.5 text-theme-primary"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
