...
const getStatusConfig = (status: string) => {
  ...
  if (normalized.includes('poison') || normalized.includes('toxic')) {
    return {
      icon: (
        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M23 12c0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2 2 .9 2 2zm-4 9c0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2 2 .9 2 2zm-4-18c1.1 0 2 .9 2 2s-.9 2-2 2-2-.9-2-2 .9-2 2-2zm-6.1 1.1c.4-.8 1.2-1.1 2.1-.7.8.4 1.1 1.2.7 2.1-.4.8-1.2 1.1-2.1.7-.8-.4-1.1-1.2-.7-2.1zM3 12c0-1.1.9-2 2-2s2 .9 2 2-.9 2-2 2-2-.9-2-2zm3.9 7.9c-.4.8-1.2 1.1-2.1.7-.8-.4-1.1-1.2-.7-2.1.4-.8 1.2-1.1 2.1-.7.8.4 1.1 1.2.7 2.1zM12 6a6 6 0 100 12 6 6 0 000-12z"></path></svg>
      ),
      style: 'bg-purple-900/20 text-purple-400 border-purple-500/30'
    };
  }
  if (normalized.includes('wound') || normalized.includes('injur') || normalized.includes('bleed') || normalized.includes('hurt')) {
    return {
      icon: (
  ...
  if (normalized.includes('health') || normalized.includes('fine') || normalized.includes('good')) {
    return {
      icon: (
  ...
  if (normalized.includes('empower') || normalized.includes('buff') || normalized.includes('strong')) {
    return {
      icon: (
  ...
  if (normalized.includes('dead') || normalized.includes('dying') || normalized.includes('unconscious')) {
    return {
      icon: (
  ...
  if (normalized.includes('tired') || normalized.includes('exhaust') || normalized.includes('fatigue')) {
    return {
      icon: (
  ...
  if (normalized.includes('terrified') || normalized.includes('fear') || normalized.includes('panic')) {
    return {
      icon: (
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center px-2 py-1.5">
        <span className="text-xs text-theme-text font-medium">{skill.name}</span>
