...
export const LocationPanel: React.FC<LocationPanelProps> = ({
  ...
  const renderLocationItem = (locationName: string, isCurrent: boolean) => {
    ...
    return (
      <div
        key={locationName}
        className={`mb-2 transition-all ${isExpanded ? "bg-theme-surface-highlight/30" : ""} rounded`}
      >
        <button
        ...
        >
          <span
            className={`font-bold tracking-wide text-xs ${isCurrent ? "text-theme-primary" : ""}`}
          >
            {locationName}
          </span>
          {isCurrent && (
    ...
    <div className="mb-6 relative">
      ...
      >
        <span className="flex items-center">
          <svg
            className="w-4 h-4 mr-2 text-theme-primary"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div className="space-y-1">
          ...
          {knownLocations.length > 0 && (
            <div className="mt-4 mb-2">
              <h4 className="uppercase text-[10px] text-theme-muted font-bold tracking-wider mb-2 border-b border-theme-border/30 pb-1">
                {t("location.known")}
              </h4>
              {knownLocations
                .filter((l) => l !== currentLocation)
                .map((loc) => renderLocationItem(loc, false))}
            </div>
          )}
        </div>
      </div>
