    ...
    <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex flex-col animate-fade-in text-theme-text font-mono">
       ...
       <div className="flex-none p-6 border-b border-theme-border bg-theme-surface flex justify-between items-center shadow-md">
          <div>
            ...
            <p className="text-xs text-theme-muted mt-1">Trace raw API requests and token usage.</p>
          </div>
          <button onClick={onClose} className="px-4 py-2 bg-theme-surface-highlight hover:bg-theme-primary hover:text-theme-bg border border-theme-border rounded transition-colors uppercase text-xs font-bold tracking-widest">
            Close Console
          </button>
       </div>
       ...
       <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-theme-bg/50">
          ...
          {logs.map((log) => {
             ...
             return (
               <div key={log.id} className={`border rounded-lg overflow-hidden shadow-sm transition-all ${isError ? 'border-red-900/50 bg-red-900/5' : 'border-theme-border bg-theme-surface'}`}>
                  {/* Log Header */}
                  <div className={`px-4 py-3 flex flex-wrap justify-between items-center border-b ${isError ? 'border-red-900/30 bg-red-900/20' : 'border-theme-border/50 bg-theme-surface-highlight/30'}`}>
                     <div className="flex items-center gap-4">
                        <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${isError ? 'bg-red-500 text-white' : 'bg-theme-primary text-theme-bg'}`}>
                           {log.provider}
                        </span>
                        <span className="text-sm font-bold text-theme-text">{log.model}</span>
                        <span className="text-xs text-theme-muted font-normal">{log.endpoint}</span>
                     </div>
                  ...
                  <div className="p-4 grid grid-cols-1 lg:grid-cols-2 gap-6">
                     <div className="space-y-2 min-w-0">
                        <label className="text-[10px] uppercase tracking-widest text-green-500 font-bold block">Request Payload</label>
                  ...
                  {log.usage && (
                     <div className="px-4 py-2 bg-theme-surface-highlight/10 border-t border-theme-border/30 flex justify-end gap-6 text-xs text-yellow-500/80">
                        <span><strong className="text-yellow-500">Prompt:</strong> {log.usage.promptTokens}</span>
                        <span><strong className="text-yellow-500">Completion:</strong> {log.usage.completionTokens}</span>
                        <span className="px-2 py-0.5 bg-yellow-900/20 border border-yellow-700/30 rounded"><strong className="text-yellow-500">Total:</strong> {log.usage.totalTokens}</span>
                     </div>
                  )}
               </div>
             );
