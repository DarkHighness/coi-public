...
export const StatisticsTab: React.FC<StatisticsTabProps> = ({ gameState, aiSettings }) => {
  ...
  const handleRebuildIndex = useCallback(async () => {
    ...
    try {
      ...
      if (ragService) {
        ...
        const storyNodeIds = Object.keys(gameState.nodes).slice(-50).map((id) => `story:${id}`);
      ...
      setError(t("ragDebugger.rebuildFailed", "Failed to rebuild index") + ": " + errorMessage);
    } finally {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-theme-surface-highlight/50 border border-theme-border rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-theme-primary">{indexStats.documentCount}</div>
            <div className="text-xs text-theme-muted uppercase tracking-wider mt-1">
              {t("ragDebugger.totalDocs", "Total Documents")}
            </div>
          </div>
          <div className="bg-theme-surface-highlight/50 border border-theme-border rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-theme-primary">{indexStats.storageDocuments}</div>
            <div className="text-xs text-theme-muted uppercase tracking-wider mt-1">
              {t("ragDebugger.storageDocs", "Storage")}
            </div>
          </div>
          <div className="bg-theme-surface-highlight/50 border border-theme-border rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-theme-primary">{indexStats.memoryDocuments}</div>
            <div className="text-xs text-theme-muted uppercase tracking-wider mt-1">
        ...
        <div className="bg-theme-surface-highlight/50 border border-theme-border rounded-lg p-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-theme-muted">{t("ragDebugger.provider", "Provider")}:</span>
              <span className="ml-2 text-theme-text font-mono">{indexStats.provider}</span>
            </div>
            <div>
              <span className="text-theme-muted">{t("ragDebugger.saveId", "Save ID")}:</span>
              <span className="ml-2 text-theme-text font-mono">{indexStats.currentSaveId || "N/A"}</span>
            </div>
            <div>
              <span className="text-theme-muted">{t("ragDebugger.initialized", "Initialized")}:</span>
              <span className={`ml-2 font-mono ${indexStats.isInitialized ? "text-green-400" : "text-red-400"}`}>
