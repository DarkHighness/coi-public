...
export const ActionPanel: React.FC<ActionPanelProps> = ({
  ...
  const getChoiceLabel = (choice: any): string => {
    ...
    if (typeof choice === 'object' && choice !== null) {
      ...
      return choice.choice || choice.text || choice.label || JSON.stringify(choice);
    }
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          {!gameState.isProcessing && !isTranslating && availableChoices.length > 0 && (
            <div className="animate-fade-in-up">
               ...
               {/* Choices List */}
               <div className={`flex flex-wrap gap-2 justify-center transition-all duration-300 overflow-hidden ${isChoicesExpanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 md:max-h-none md:opacity-100'}`}>
                  {availableChoices.map((rawChoice, idx) => {
                    const label = getChoiceLabel(rawChoice);
                    return (
                      <div key={idx} className="group relative inline-flex rounded-full shadow-sm hover:shadow-[0_0_15px_rgba(var(--theme-primary),0.4)] transition-all duration-300">
                        <button
