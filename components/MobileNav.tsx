...
export const MobileNav: React.FC<MobileNavProps> = ({ currentTab, setTab, language }) => {
  ...
  const tabs: { id: MobileTab; label: string; icon: React.ReactNode }[] = [
    ...
    },
    {
      ...
      icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7"></path></svg>
    }
  ];
       ...
       <div className="flex justify-around items-center h-16">
          {tabs.map(tab => {
             ...
             return (
                ...
                >
                   {tab.icon}
                   <span className="text-[10px] uppercase tracking-wide font-bold mt-1">{tab.label}</span>
                   {isActive && <div className="absolute bottom-0 w-1/3 h-0.5 bg-theme-primary rounded-t-full"></div>}
                </button>
             )
          })}
       </div>
