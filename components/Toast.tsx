        ...
        <div>
          ...
          >
            <span className="text-sm font-bold uppercase tracking-widest flex items-center gap-2">
              {icon}
              {toast.message}
              <span className="text-xs opacity-70 normal-case">({toast.items!.length})</span>
            </span>
            <svg
            ...
            <div className="px-4 pb-3 pt-1 border-t border-current/20">
              <ul className="space-y-1">
                {toast.items!.map((item, idx) => (
                  <li key={idx} className="text-xs opacity-90 flex items-center gap-1">
...
export const useToastManager = () => {
  ...
  const pushStateChangeToasts = useCallback((changes: StateChanges, t: (key: string) => string) => {
    const mobile = isMobile();

    // Collect all changes for potential grouping on mobile
    const allChanges: Array<{ msg: string; type: ToastItem["type"]; items: string[] }> = [];
    ...
    if (changes.locationsDiscovered && changes.locationsDiscovered.length > 0) {
      allChanges.push({
    ...
    if (mobile && allChanges.length > 1) {
      ...
      const totalItems = allChanges.reduce((sum, c) => sum + c.items.length, 0);
