...
const ANIMATION_TEMPLATES = [
  { id: 'cinematic', icon: '🎥', prompt: "Cinematic slow zoom, high drama, detailed texture, 4k" },
  { id: 'nature', icon: '🌿', prompt: "Wind blowing gently, leaves rustling, subtle organic movement, atmospheric" },
  { id: 'magic', icon: '✨', prompt: "Glowing particles, swirling magical mist, ethereal energy flow, soft lighting" },
  { id: 'action', icon: '⚡', prompt: "Dynamic camera angle, fast movement, intense energy, cinematic blur" },
  { id: 'portrait', icon: '👤', prompt: "Subtle facial expression change, breathing, blinking, focus on eyes" },
];

export const MagicMirror: React.FC<MagicMirrorProps> = ({ isOpen, onClose, initialImage, language, themeFont = 'font-fantasy' }) => {
  const [image, setImage] = useState<string | null>(null);
  const [prompt, setPrompt] = useState("Cinematic slow zoom, high drama, detailed texture, 4k");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-theme-surface border border-theme-border rounded-sm max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-[0_0_60px_rgba(var(--theme-primary),0.15)] flex flex-col relative transition-colors duration-500">
        ...
        <div className="p-6 border-b border-theme-border flex justify-between items-start bg-gradient-to-b from-theme-surface-highlight to-theme-surface">
          <div>
            <h2 className={`text-3xl text-transparent bg-clip-text bg-gradient-to-r from-theme-text via-theme-primary to-theme-primary-hover ${themeFont} tracking-wide drop-shadow-sm`}>
              {t.animatorTitle}
            </h2>
          ...
          <div className="relative aspect-video bg-black/40 rounded border border-theme-border flex items-center justify-center overflow-hidden group shadow-inner">
            {videoUrl ? (
              ...
              <div className="text-center p-8 text-theme-muted/40">
                ...
                <p className={`${themeFont} tracking-wider text-lg`}>{t.dragDrop}</p>
              </div>
            )}
          ...
          <div className="relative z-10">
             ...
             <div className="flex flex-wrap gap-2">
                {ANIMATION_TEMPLATES.map(tmpl => (
                  ...
                  >
                    <span>{tmpl.icon}</span>
                    <span>{(t.styleLabels as any)?.[tmpl.id] || tmpl.id}</span>
                  </button>
                ))}
             </div>
          </div>
            ...
            {!hasKey ? (
              <div className="bg-theme-surface-highlight border border-theme-border p-5 text-center">
                ...
                <div className="mt-3 text-xs text-theme-muted">
                  <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noreferrer" className="underline hover:text-theme-primary">{t.billingInfo}</a>
                </div>
              </div>
            ) : (
              <button 
              ...
              >
                ...
                {loading ? (
                  <>
                    ...
                    <span className={`${themeFont} tracking-widest uppercase`}>{t.processing}</span>
                  </>
                ) : (
                  <>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
                    <span className={`${themeFont} tracking-widest uppercase`}>{t.animateAction}</span>
                  </>
                )}
              </button>
            )}
