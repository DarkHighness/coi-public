...
type EffectType = 'rain' | 'snow' | 'fog' | 'flicker' | 'embers' | 'sunny' | null;

export const EnvironmentalEffects: React.FC<EnvironmentalEffectsProps> = ({ currentText, imagePrompt, theme, environment, backgroundImage }) => {
  const [effect, setEffect] = useState<EffectType>(null);

  useEffect(() => {
    // Combine texts for analysis
    const analysisText = `${currentText} ${imagePrompt || ''} ${environment || ''}`.toLowerCase();
    ...
    else if (analysisText.includes('sun') || analysisText.includes('beam') || analysisText.includes('dust mote') || analysisText.includes('pollen')) {
