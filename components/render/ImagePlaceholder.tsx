      ...
      {isGenerating ? (
        ...
        >
          <div className="relative">
            <svg className="w-12 h-12 text-theme-muted group-hover:text-theme-primary transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <span className={`text-xs uppercase tracking-widest transition-colors ${hasFailed ? 'text-red-400 group-hover:text-red-500' : 'text-theme-muted group-hover:text-theme-primary'}`}>
            {hasFailed ? (labelFailed || t('generationFailed')) : t('clickToGenerate')}
          </span>
        </button>
      ) : (
