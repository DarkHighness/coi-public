...
export const handleReadMarkdown: VfsToolHandler = (args, ctx) =>
  runWithStructuredErrors("vfs_read_markdown", args, () => {
    ...
    if (singleSectionTokenFailure) {
      return createReadLimitError(
        "markdown",
        `section "${singleSectionTokenFailure.section.index}" estimates ${singleSectionTokenFailure.sectionEstimatedTokens} tokens, exceeding budget ${readTokenBudget}`,
        typedArgs.path,
        {
          tokenBudget: readTokenBudget,
    ...
    if (totalEstimatedTokens > readTokenBudget) {
      return createReadLimitError(
        "markdown",
        `combined markdown section payload estimates ${totalEstimatedTokens} tokens, exceeding budget ${readTokenBudget}`,
        typedArgs.path,
        {
          tokenBudget: readTokenBudget,
