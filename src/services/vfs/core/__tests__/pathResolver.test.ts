...
describe("pathResolver", () => {
  it("resolves current alias paths into canonical fork/shared namespaces", () => {
    const world = resolveVfsPath("current/world/global.json", { activeForkId: 2 });
    ...
    const skills = resolveVfsPath("current/skills/README.md", { activeForkId: 2 });
  ...
  it("keeps non-active fork world files canonical unless loose mode", () => {
    ...
    expect(canonicalToLogicalVfsPath(canonical, { activeForkId: 0 })).toBe(canonical);
    expect(canonicalToLogicalVfsPath(canonical, { activeForkId: 0, looseFork: true })).toBe(
      "world/global.json",
    );
  });

  it("provides convenience helpers for logical/current paths", () => {
    expect(toLogicalVfsPath("current/summary/state.json", { activeForkId: 1 })).toBe(
      "summary/state.json",
    );
    expect(toCurrentDisplayPath("forks/1/story/summary/state.json", { activeForkId: 1 })).toBe(
      "current/summary/state.json",
    );
  });
});
