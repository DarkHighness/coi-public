...
describe("vfsElevationTokenManager", () => {
  ...
  it("tracks editor session token validity and revocation", () => {
    ...
    expect(vfsElevationTokenManager.isValidEditorSessionToken(token)).toBe(
      true,
    );
  ...
  it("reset clears all token records", () => {
    ...
    expect(vfsElevationTokenManager.consumeAiElevationToken(aiToken)).toBe(
      false,
    );
    expect(
      vfsElevationTokenManager.isValidEditorSessionToken(editorToken),
    ).toBe(false);
  });
