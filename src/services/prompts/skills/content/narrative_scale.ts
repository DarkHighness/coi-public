 ...
 * ============================================================================
 *
 * 根据 AI 在 Phase 1 选择的 narrativeScale 生成对应的运行时提示词
 */
...
export function getNarrativeScaleContent(ctx: SkillContext): string {
  ...
  switch (scale) {
    case "intimate":
      return getIntimateScaleContent(ctx);
    case "epic":
      return getEpicScaleContent(ctx);
    case "balanced":
      return getBalancedScaleContent(ctx);
    default:
      return "";
  }
}
...
/**
 * 小格局叙事 (Intimate Scale) - 日常、温情、个人
 */
function getIntimateScaleContent(ctx: SkillContext): string {
  return `
...
/**
 * 宏大叙事 (Epic Scale) - 世界级威胁、命运、史诗
 */
function getEpicScaleContent(ctx: SkillContext): string {
  return `
...
/**
 * 平衡叙事 (Balanced Scale) - 个人危机但非末日
 */
function getBalancedScaleContent(ctx: SkillContext): string {
  return `
