 ...
 */
export const renderLocationVisible: Atom<RenderLocationInput> = ({ location }) => {
  ...
  if (v.sensory) {
    ...
    if (v.sensory.lighting) sensoryParts.push(`lighting: ${v.sensory.lighting}`);
    if (v.sensory.temperature) sensoryParts.push(`temperature: ${v.sensory.temperature}`);
    if (sensoryParts.length) lines.push(`sensory: { ${sensoryParts.join(", ")} }`);
  }

  if (v.knownFeatures?.length) lines.push(`knownFeatures: ${JSON.stringify(v.knownFeatures)}`);
  if (v.resources?.length) lines.push(`resources: ${JSON.stringify(v.resources)}`);
 ...
 */
export const renderLocationHidden: Atom<RenderLocationInput> = ({ location }) => {
  ...
  if (h.hiddenFeatures?.length) lines.push(`hiddenFeatures: ${JSON.stringify(h.hiddenFeatures)}`);
...
export const renderLocationFull: Atom<RenderLocationInput> = ({ location }) => {
  ...
  if (v.knownFeatures?.length) visibleLines.push(`knownFeatures: ${JSON.stringify(v.knownFeatures)}`);
  if (v.resources?.length) visibleLines.push(`resources: ${JSON.stringify(v.resources)}`);
...
${visibleLines.join("\n")}
  ...
  if (h.fullDescription) hiddenLines.push(`fullDescription: ${h.fullDescription}`);
  if (h.hiddenFeatures?.length) hiddenLines.push(`hiddenFeatures: ${JSON.stringify(h.hiddenFeatures)}`);
  if (h.dangers?.length) hiddenLines.push(`dangers: ${JSON.stringify(h.dangers)}`);
