...
const minimalEntity = `
    ...
    **THE PRINCIPLE**:
    ...
    - **Outline Continuity**: Do not re-create entities that were part of your character creation or world foundation. If the Outline made it, YOU usually made it. Read \`current/outline/outline.json\` via \`vfs_read_json\` and \`workspace/PLAN.md\` via \`vfs_read_markdown\` (or bounded \`vfs_read_lines\`), then check existing files under \`current/world/\`.
    ...
    **MANDATORY "INVESTIGATIVE SEARCH" WORKFLOW**:
    ...
    3. **SEARCH (Text Scan)**: Use \`vfs_search\` or \`vfs_search\` on \`current/world/<type>/\`, \`current/outline/outline.json\`, and \`workspace/PLAN.md\`.
