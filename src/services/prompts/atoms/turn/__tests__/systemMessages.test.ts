...
describe("turn system message atoms", () => {
  it("normal turn instruction uses VFS-only workflow", () => {
    const content = normalTurnInstruction({ finishToolName: "finish_turn" });
  ...
  it("no-tool-call error references VFS tools", () => {
    const content = noToolCallError({ finishToolName: "finish_turn" });
