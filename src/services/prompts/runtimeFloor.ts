...
- VFS primer:
  ...
  - \`vfs_vm\`: multi-step JavaScript orchestrator. MUST be the only top-level tool call. Exactly one JavaScript script per call. No recursion, no \`import\`/\`eval\`/\`Function\`/\`globalThis\`/\`window\`. Runtime enforces bounded inner tool calls and script length. Finish at most once, last.
