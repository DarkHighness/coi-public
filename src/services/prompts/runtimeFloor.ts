...
- VFS primer:
  ...
  - \`vfs_vm\`: multi-step JavaScript orchestrator. MUST be the only top-level tool call. Exactly one JavaScript script per call. No recursion, no \`import\`/\`eval\`/\`Function\`/\`globalThis\`/\`window\`. Inside script, call injected helpers directly by tool name (for example \`await vfs_read_chars({...})\`); NEVER use \`VFS.read(...)\` or any \`VFS.*\` namespace. Runtime enforces bounded inner tool calls and script length. Finish at most once, last.
