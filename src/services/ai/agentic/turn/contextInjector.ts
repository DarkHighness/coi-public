    ...
    history.push(
      createUserMessage(
        [
          ...
          "  3) Re-read only missing sections/anchors needed for recovery (or when `[SYSTEM: EXTERNAL_FILE_CHANGES]` is present): start with `current/conversation/index.json` + files named in error/retry targets.",
          "  4) Retry one corrected call.",
          "  5) If the same code repeats twice, narrow scope and report blocker instead of forcing progress.",
        ].join("\n"),
      ),
    );
  }
