...
const buildColdStartSkillHintMessage = (
  ...
  return createUserMessage(
    [
      ...
      "Strongly recommended right after mandatory reads:",
      "- `current/skills/index.json` (discover relevant skills first)",
      "- Read 1-3 additional skill files that match this session's active domain/theme/mechanics needs.",
      ...
      "- Reuse those skill docs across turns; avoid re-reading the same skill every turn.",
      "- Re-read a previously read skill only when scope was insufficient or when `[SYSTEM: EXTERNAL_FILE_CHANGES]` explicitly marks that path.",
