...
export const repairGameState = (state: GameState): GameState => {
  ...
  if (!state.nextIds) {
     state.nextIds = {
        item: 1, npc: 1, location: 1, knowledge: 1, quest: 1, faction: 1,
        timeline: 1, causalChain: 1, skill: 1, condition: 1, hiddenTrait: 1
     };
  }
  ...
  types.forEach(({ type, list, idKey }) => {
    const keyMap: Record<EntityType, keyof typeof state.nextIds> = {
        inventory: "item", npc: "npc", location: "location", quest: "quest",
        knowledge: "knowledge", faction: "faction", timeline: "timeline",
        causalChain: "causalChain", skill: "skill", condition: "condition",
 ...
 */
export const syncIdCounter = (state: GameState, type: EntityType, manualId: string): void => {
