...
import type { AISettings, FunctionConfig } from "../../types";
...
export const SettingsModels: React.FC<SettingsModelsProps> = ({
  ...
  const providerManagedMaxTokens = extra.providerManagedMaxTokens ?? true;
  ...
  return (
    <div className="flex flex-col gap-6 animate-slide-in">
      ...
      <div className="order-6 py-4 border-b border-theme-border/25 space-y-4">
        ...
        <div className="flex items-start justify-between gap-4 py-2 border-t border-theme-border/15">
          <div>
            <div className="text-xs text-theme-text">
              {t("settings.extra.providerManagedMaxTokens") ||
                "Provider-managed Max Tokens"}
            </div>
            <div className="text-[10px] text-theme-muted">
              {t("settings.extra.providerManagedMaxTokensHelp") ||
                "Enabled by default. When on, requests omit maxTokens/max_tokens/max_completion_tokens/maxOutputTokens and let provider defaults apply."}
            </div>
          </div>
          <button
            aria-label={
            ...
            onClick={() =>
              updateExtra("providerManagedMaxTokens", !providerManagedMaxTokens)
            }
            className={`w-10 h-5 rounded-full relative transition-colors ${
              providerManagedMaxTokens ? "bg-green-500" : "bg-theme-border"
            }`}
          >
            <span
              className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${
                providerManagedMaxTokens ? "translate-x-5" : ""
              }`}
            />
          </button>
        </div>
