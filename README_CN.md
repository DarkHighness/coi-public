...
## 🧠 上下文与摘要策略（Compact / Query Summary / Cleanup）

为兼顾成本、稳定性与长期一致性，系统采用分层策略：

- **Compact（最高优先级）**：
  在原始对话上下文中直接生成 summary，这是默认且最经济的路径。
- **Query Summary（Fallback）**：
  当 Compact 因上下文溢出或历史参数异常无法完成时，自动降级到锚点检索模式：
  先确定当前回合/摘要锚点，再通过 `query_turn`、`query_summary` 与 `vfs_read` 等工具重建摘要。
- **Cleanup（一致性修复）**：
  以当前回合与摘要锚点为基准，指导 AI 通过“读取→修复→复核”的迭代流程，修复实体逻辑不一致、重复 NPC/实体等问题。
...
- 上下文窗口覆盖现在按 **provider + model** 作用域保存（不再是全局）。
- 系统按以下顺序解析上下文窗口：
  1. 当前 `providerId + modelId` 的手动覆盖
  2. Provider 返回的模型元数据（如果可用）
  3. 内置默认映射（维护的快照）
  4. 回退值 `32000`
- 在 **设置 → 模型** 中可点击 **复制默认映射**，把内置映射 JSON 复制到剪贴板。
